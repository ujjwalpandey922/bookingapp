 ...
 const {data,loading,error} = useFetch(`/api/hotels/getrooms/${hotelId}`);
  ...
  const handleSelect =(e)=>{
   ...
   const value = e.target.value;
    setSelectedRooms(checked?[...selectedRooms,value]:selectedRooms.filter((e)=>e != value))
  }
  // getdateinrange -------------------
  ...
  const allDates = getDateInRange(state.dates[0].startDate,state.dates[0].endDate);
  //handle CLICK --------------
  ...
  const isAvailable =(roomNumber)=>{
    const isFound = roomNumber.unavailableDates.some((e)=> allDates.includes(new Date(e).getTime()))
 ...
 return (
    <div className="reserve">
      <div className="rcontainer">
        <AiFillCloseCircle onClick={()=>setOpenModal(false)} className="rclose"/>
        <em>Select your hotel room</em>
        <div className="rrooms">
            {data.map((item) => (
          <div className="rItem" key={item._id}>
            <div className="rItemInfo">
              <div className="rTitle">🛌 <u>{item.title} </u></div>
